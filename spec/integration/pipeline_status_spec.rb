...
describe "Pipeline Status Test with 100 nodes" do
  ...
  it "executes all 100 nodes in sequence, calls all callbacks, and sets all statuses to completed" do
    ...
    expect($pipeline_status_callbacks_runs.size).to eq(100), "Expected 100 callbacks, got #{$pipeline_status_callbacks_runs.size}"
    expect($pipeline_status_callbacks_runs.sort).to eq(expected), "Callbacks were not called in correct order or some are missing"
    ...
    if database_available
      pipeline = Sidekiq::SidekiqPipeline.for('testpipeline')
      ...
      expect(all_completed).to be(true), "Not all nodes are completed. Statuses: #{pipeline_nodes.map { |n| "#{n.node_name}:#{n.status}" }.inspect}"
      ...
      all_completed_symbols_check = pipeline.sidekiq_pipeline_nodes.map { |node| node.status.to_sym }.all? { |x| x == :completed }
